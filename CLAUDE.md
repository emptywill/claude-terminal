...
### Technical
- **Session info tooltip** - GET /api/servers/:id/sessions/:session/cwd endpoint queries tmux with `list-panes -F "#{pane_current_path}"` to populate hover tooltip with CWD and session metadata
