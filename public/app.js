...
(function() {
    ...
    async function loadSkills() {
        ...
        if (!currentSession || !currentServerId) {
            skillsList.innerHTML = '<div class="empty-state">No active session. Connect to a session to view skills.</div>';
            return;
        }

        skillsList.innerHTML = '<div class="empty-state">Loading skills...</div>';

        try {
            const response = await fetch(`/api/servers/${currentServerId}/sessions/${encodeURIComponent(currentSession)}/skills`);
            if (response.ok) {
                const data = await response.json();
                if (data.skills && data.skills.length > 0) {
                    renderSkills(data.skills);
                } else {
                    skillsList.innerHTML = '<div class="empty-state">No skills installed. Run <code>claude plugin install</code> to add skills.</div>';
                }
            } else {
                skillsList.innerHTML = '<div class="empty-state">Failed to load skills</div>';
            }
        } catch (error) {
            console.error('Failed to load skills:', error);
            skillsList.innerHTML = '<div class="empty-state">Error loading skills</div>';
        }
    }
        ...
        skillsList.innerHTML = skills.map(skill => `
            <div class="skill-item">
                <div class="skill-header">
                    ...
                    <button class="skill-launch" data-skill="${skill.name}">Launch</button>
                </div>
                ${skill.description ? `<div class="skill-description">${skill.description}</div>` : ''}
                ${skill.marketplace ? `<div class="skill-marketplace">📦 ${skill.marketplace}</div>` : ''}
            </div>
        `).join('');

        // Add click handlers for launch buttons
        skillsList.querySelectorAll('.skill-launch').forEach(btn => {
            btn.addEventListener('click', () => {
                const skillName = btn.dataset.skill;
                launchSkill(skillName);
            });
        });
    }

    function launchSkill(skillName) {
        if (!term) return;

        // Append slash command to terminal input (don't press enter)
        ...
        showToast(`Added /${skillName} to terminal`, 'info', 2000);
    }
    ...
    async function loadFiles() {
        ...
        if (!currentSession || !currentServerId) {
            filesTree.innerHTML = '<div class="empty-state">No active session. Connect to a session to view files.</div>';
            return;
        }

        filesTree.innerHTML = '<div class="empty-state">Loading files...</div>';

        try {
            const response = await fetch(`/api/servers/${currentServerId}/sessions/${encodeURIComponent(currentSession)}/files`);
            if (response.ok) {
                const data = await response.json();
                if (data.files && data.files.length > 0) {
                    renderFileTree(data.files, data.cwd);
                } else {
                    filesTree.innerHTML = '<div class="empty-state">No files found</div>';
                }
            } else {
                filesTree.innerHTML = '<div class="empty-state">Failed to load files</div>';
            }
        } catch (error) {
            console.error('Failed to load files:', error);
            filesTree.innerHTML = '<div class="empty-state">Error loading files</div>';
        }
    }

    function renderFileTree(files, cwd) {
        const filesTree = document.getElementById('filesTree');

        let html = `<div class="file-tree-item directory" data-indent="0">
            <span class="file-tree-icon">📁</span>
            <span class="file-tree-name">${cwd || 'Working Directory'}</span>
        </div>`;

        html += files.map(file => {
            const icon = file.type === 'directory' ? '📁' : '📄';
            const status = file.gitStatus ? `<span class="file-tree-status">${file.gitStatus}</span>` : '';
            return `
                <div class="file-tree-item ${file.type}" data-indent="1">
