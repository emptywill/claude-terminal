...
// Get installed Claude Code skills
app.get('/api/servers/:id/sessions/:session/skills', (req, res) => {
    const { id, session } = req.params;
    ...
    // Find all skills from installed plugin bundles
    const cmd = `
        ...
        for SKILL_DIR in ~/.claude/plugins/cache/*/*/unknown/skills/*; do
            if [ -f "$SKILL_DIR/SKILL.md" ]; then
                SKILL_NAME=$(basename "$SKILL_DIR")
                BUNDLE_PATH=$(dirname $(dirname "$SKILL_DIR"))
                MARKETPLACE=$(basename $(dirname $(dirname "$BUNDLE_PATH")))
                ...
                SKILLS_JSON="$SKILLS_JSON{\"name\":\"$SKILL_NAME\",\"description\":\"$DESCRIPTION\",\"marketplace\":\"$MARKETPLACE\"}"
            fi
        done
    ...
    if (server.authType === 'local') {
        // Run command through tmux to access host filesystem
        const escapedCmd = cmd.replace(/\n/g, ' ').replace(/'/g, "'\\''");
        const tmuxCmd = `tmux run-shell -t "${session}" "bash -c '${escapedCmd}'" 2>&1`;

        exec(tmuxCmd, { timeout: 5000 }, (error, stdout, stderr) => {
            if (error) {
                console.error('Skills tmux command error:', error, stderr);
            }
            console.log('Skills command output:', stdout.substring(0, 200) + '...');
            try {
                const skills = JSON.parse(stdout.trim() || '[]');
                console.log('Parsed skills:', skills.length, 'skills');
                res.json({ skills });
            } catch (e) {
...
// Get file tree for current session
app.get('/api/servers/:id/sessions/:session/files', (req, res) => {
    const { id, session } = req.params;
    ...
    const cmd = `CWD=$(tmux display-message -t "${session}" -p "#{pane_current_path}" 2>/dev/null); echo "CWD:$CWD"; cd "$CWD" 2>/dev/null && ls -1Ap 2>/dev/null || echo ""`;
    ...
    if (server.authType === 'local') {
        ...
        exec(tmuxCmd, { timeout: 5000 }, (error, stdout, stderr) => {
            if (error) {
                console.error('Files tmux command error:', error, stderr);
                return res.json({ files: [], cwd: '', error: error.message });
            }
            console.log('Files command output:', stdout);
            const lines = stdout.trim().split('\n').filter(l => l);
            const cwdLine = lines.find(l => l.startsWith('CWD:'));
            const cwd = cwdLine ? cwdLine.replace('CWD:', '') : '';
            const files = lines
                .filter(l => !l.startsWith('CWD:') && l)
                .map(name => ({
                    name: name.replace(/\/$/, ''),
                    type: name.endsWith('/') ? 'directory' : 'file'
                }));
            console.log('Parsed files:', files.length, 'files from', cwd);
            res.json({ files, cwd });
        });
    } else {
        const conn = new Client();
